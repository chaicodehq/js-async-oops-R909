...
export class DabbaService {
  ...
  addCustomer(name, address, mealPreference) {
    const validMeals = ["veg", "nonveg", "jain"];

    if (!validMeals.includes(mealPreference)) {
      return null;
    }

    const duplicate = this.customers.find((item) => item.name === name);
    if (duplicate) {
      return null;
    }
  ...
  createDeliveryBatch() {
    const activeCustomers = this.customers.filter((item) => item.active);

    if (activeCustomers.length === 0) {
      return [];
    }

    // Reset delivered for all active customers before creating batch
    ...
    return activeCustomers.map((item) => ({
      customerId: item.id,
      name: item.name,
  ...
  markDelivered(customerId) {
    const customer = this.customers.find(
      (item) => item.id === customerId && item.active
    );
  ...
  getDailyReport() {
    ...
    const delivered = activeCustomers.filter((item) => item.delivered).length;
    const pending = activeCustomers.filter((item) => !item.delivered).length;

    const mealBreakdown = {
      veg: activeCustomers.filter((item) => item.mealPreference === "veg").length,
      nonveg: activeCustomers.filter((item) => item.mealPreference === "nonveg").length,
      jain: activeCustomers.filter((item) => item.mealPreference === "jain").length,
    };
  ...
  getCustomer(name) {
    return this.customers.find((item) => item.name === name) || null;
  }
}
