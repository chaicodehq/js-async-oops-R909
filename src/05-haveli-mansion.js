...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    const alreadyExists = this.#residents.some(
  ...
  verifyAccess(name) {
    const exists = this.#residents.some((resident) => resident.name === name);
  ...
  isResident(name) {
    return this.#residents.some((resident) => resident.name === name);
  }
}